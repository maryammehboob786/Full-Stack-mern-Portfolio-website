...
const AnimatedTechStack = () => {
  ...
  const TechCard = ({ tech, index }) => (
    ...
    >
      <div className={`
        absolute inset-0 bg-gradient-to-r from-pink-500/20 to-purple-500/20 
        rounded-xl blur-lg transition-opacity duration-300
        ${hoveredIndex === index ? 'opacity-100' : 'opacity-0'}
      `}></div>
      
      <div className={`
        relative z-10 p-6 rounded-xl backdrop-blur-md
      ...
      `}>
        <div className="relative">
          <img
            ...
            alt={`${tech.name} logo`}
            className={`
              w-16 h-16 object-contain transition-all duration-500 animate-spin-slow
              ${hoveredIndex === index ? 'scale-110' : 'scale-100'}
            `}
          />
          <div className={`
            absolute inset-0 bg-white/20 rounded-full blur-md transition-opacity duration-300
            ${hoveredIndex === index ? 'opacity-100' : 'opacity-0'}
          `}></div>
        </div>
        
        <span className={`
          block text-center mt-4 font-medium transition-all duration-300
